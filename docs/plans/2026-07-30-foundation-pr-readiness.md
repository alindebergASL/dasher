...
1. Create `docs/status/2026-07-30-foundation-readiness.md` with sections:
   ...
   - **Known deltas at the pre-merge checkpoint**: the 7-task plan landed as one squashed commit; `docs/architecture/ADR-001-foundation.md` and `ADR-002` were both `Status: Proposed`; ADR-001's repository layout was aspirational (`apps/worker`, `packages/ui|provenance|model-gateway|sandbox-contract`, `infra/*` did not exist); package-level `lint` scripts actually ran `tsc --noEmit` (root `pnpm lint` was the only ESLint entry point). ADR-001 transitioned to Accepted after foundation PR #1 merged and CI passed. ADR-002 was accepted later as a gated target boundary by the architecture synthesis; neither transition claims those future systems are implemented.
