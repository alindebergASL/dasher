...
| ---------------------------- | ---------------------------------------------- | -------------------- |
| Tenant isolation             | forced RLS, composite FKs, restricted roles    | no                   |
...
This ADR is `Proposed`. Accepting it requires an owner decision, and separately
declaring a Freeze Point requires a second one. Until both, ADR-003's current
language governs and migrations `0001`–`0008` remain immutable.

Accepting this ADR does not by itself authorize a squash. It establishes that a
squash is permissible before the Freeze Point; performing one requires its own
reviewed plan, a full adversarial gate run against the baseline, and the archival
of superseded files.
