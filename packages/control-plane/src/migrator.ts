...
function canonicalIndexSignatureWithClosure(signature: string): string {
  ...
  const keyExpressionArray = parts[11];
...
function exactCatalogContract(
  ...
  const indexes = [
    `dasher_meta|schema_migrations|schema_migrations_filename_key|btree|true|false|true|true|true|false|false|<none>|1|1|{filename}|{}|{text_ops}|{pg_catalog.default}|0`,
    `dasher_meta|schema_migrations|schema_migrations_pkey|btree|true|true|true|true|true|false|false|<none>|1|1|{sequence}|{}|{int4_ops}|{<none>}|0`,
...
/** Test-only snapshot of the exact canonical-0002 catalog contract. */
export function getCanonical0002ExactCatalogContractForTests(
  ownerName: string,
): unknown {
  const journalRows = modeledSuccessorFiles.slice(0, 2).map(
    (file, index) =>
      ({
...
/** Test-only execution of the same exact catalog comparison used by the runner. */
export async function exactCatalogMatchesForTests(
  client: MigrationClient,
  expected: unknown,
  ownerName: string,
): Promise<boolean> {
